#############################################模块引进#################################################
from new_nn import Ui_MainWindow
...
class UnitConv:
    ...
    g_length_output = {'分米（dm）': 0.0,
                       '厘米（cm）': 0.0, '毫米（mm）': 0.0, '米（m）': 0.0}

    g_rho_output = {'克/立方厘米（g/cm3）': 0.0, '千克/立方米（kg/m3）': 0.0}
    PA_temperature_output = {'摄氏度（℃）': 0.0, '华氏度（F）': 0.0, '开尔文（K）': 0.0}
    Tamb_temperature_output = {'摄氏度（℃）': 0.0, '华氏度（F）': 0.0, '开尔文（K）': 0.0}
    Tgas_temperature_output = {'摄氏度（℃）': 0.0, '华氏度（F）': 0.0, '开尔文（K）': 0.0}

    PA_pressure_output = {'帕斯卡（Pa）': 0.0, '千帕（kPa）': 0.0,
                          '兆帕（MPa）': 0.0, '标准大气压（atm）': 0.0, '巴（Bar）': 0.0}
    NN_pressure_output = {'帕斯卡（Pa）': 0.0, '千帕（kPa）': 0.0,
                          '兆帕（MPa）': 0.0, '标准大气压（atm）': 0.0, '巴（Bar）': 0.0}
    ...
    def length_Unit_conversion(self, input_data, input_line, length_output):
        ...
        Args:
            ...
            length_output ([dictionary]): [储存不同单位的长度的字典]
        """
        ...
        try:
            ...
            QMessageBox.critical(
                MainWindow, "错误", '请输入合适的长度计算参数', QMessageBox.Yes, QMessageBox.Yes)
            global g_length_output
            g_length_output = {'分米（dm）': 0.0,
                               '厘米（cm）': 0.0, '毫米（mm）': 0.0, '米（m）': 0.0}
    ...
    def _pressure_Unit_conversion(self, input_line, pressure_output):
        ...
        Args:
            ...
            pressure_output ([dictionary]): [储存不同单位的压力的字典]
        """
        try:
            ...
            QMessageBox.critical(
                MainWindow, "错误", '请输入合适的压力计算参数', QMessageBox.Yes, QMessageBox.Yes)
    ...
    def pressure_Unit_conversion(self, input_data, input_line, pressure_output):
        ...
        try:
            if input_data.currentText() == "帕斯卡（Pa）":
                ...
                pressure_output["标准大气压（atm）"] = float(
                    input_line.text())*9.8692e-6
    ...
    def _temperature_unit_conversion(self, input_line, temperature_output):
        ...
        Args:
            input_line ([str]): [CoolProp的输出结果]
            temperature_output ([dictionary]): [储存不同单位的温度的字典]
        """
        try:
            temperature_output["摄氏度（℃）"] = float(input_line) - 273.15
            temperature_output["华氏度（F）"] = 32 + \
                1.8 * (float(input_line) - 273.15)
            temperature_output["开尔文（K）"] = float(input_line)
        except:
            QMessageBox.critical(
                MainWindow, "错误", '请输入合适的温度计算参数', QMessageBox.Yes, QMessageBox.Yes)
            global PA_temperature_output, Tamb_temperature_output, Tgas_temperature_output
            PA_temperature_output = {'摄氏度（℃）': 0.0, '华氏度（F）': 0, '开尔文（K）': 0}
            Tamb_temperature_output = {'摄氏度（℃）': 0.0, '华氏度（F）': 0, '开尔文（K）': 0}
            Tgas_temperature_output = {'摄氏度（℃）': 0.0, '华氏度（F）': 0, '开尔文（K）': 0}
    ...
    def temperature_unit_conversion(self, input_data, input_line, temperature_output):
        ...
        try:
            if input_data.currentText() == "摄氏度（℃）":
                ...
                temperature_output["开尔文（K）"] = float(
                    input_line.text()) + 273.15

            elif input_data.currentText() == "华氏度（F）":
                temperature_output["摄氏度（℃）"] = (
                    float(input_line.text())-32)/1.8
                temperature_output["华氏度（F）"] = float(input_line.text())
                temperature_output["开尔文（K）"] = (
                    float(input_line.text())-32)/1.8+273.15

            elif input_data.currentText() == "开尔文（K）":
                temperature_output["摄氏度（℃）"] = float(input_line.text())-273.15
                temperature_output["华氏度（F）"] = 32 + 1.8 * \
                    (float(input_line.text())-273.15)
    ...
    def _rho_unit_conversion(self, input_line, rho_output):
        ...
        Args:
            input_line ([str]): [CoolProp的输出结果]
            rho_output ([dictionary]): [储存不同单位的密度的字典]
        """
        try:
            ...
            QMessageBox.critical(
                MainWindow, "错误", '请输入合适的密度计算参数', QMessageBox.Yes, QMessageBox.Yes)
            global g_rho_output
            g_rho_output = {'克/立方厘米（g/cm3）': 0.0, '千克/立方米（kg/m3）': 0.0}
...
def EOS_model():
    ...
    try:
        ...
        if ui.PA_P_in.isEnabled() == False:
            ...
            output = str(EOSS.P(float(PA_temperature_output["开尔文（K）"]), float(
                g_rho_output["千克/立方米（kg/m3）"])))
        ...
        if ui.PA_T_in.isEnabled() == False:
            ...
            output = str(EOSS.T(float(PA_pressure_output["帕斯卡（Pa）"]), float(
                g_rho_output["千克/立方米（kg/m3）"])))
            conv._temperature_unit_conversion(output, PA_temperature_output)
            conv.temperature_refresh(
                ui.PA_T, ui.PA_T_in, PA_temperature_output)

        if ui.PA_rho_in.isEnabled() == False:

            output = str(EOSS.rho(float(PA_temperature_output["开尔文（K）"]), float(
                PA_pressure_output["帕斯卡（Pa）"])))
        ...
        QMessageBox.critical(MainWindow, "错误", str(Argument).split(":")[
                             0], QMessageBox.Yes, QMessageBox.Yes)
    except KeyError:
        QMessageBox.critical(MainWindow, "错误", '请输入合适的计算参数',
                             QMessageBox.Yes, QMessageBox.Yes)


def printf(output_message):  
    """[用于向textbroswer屏幕输出信息，可以替代print函数]

    Args:
        output_message ([str]): [需要向外输出的信息]
    """
...
def NN_model():
    ...
    Raises:
        KeyError: [如果不输入初始参数就开始计算将会提示错误，需要输入计算初始参数，其他由于coolprop造成的错误将会直接返回错误信息]
    """
    try:
        if set((g_length_output.values() and g_rho_output.values() and PA_pressure_output.values()
                and PA_temperature_output.values() and Tamb_temperature_output.values()
                and Tgas_temperature_output.values() and NN_pressure_output.values())) == {0.0}:
            raise KeyError
        ...
        class effective:
            def __init__(self, gas_eff=None, orifice_eff=None, Veff=None):
                ...
                self.mdot, self.d = orifice_eff.mdot(
                    self.rho, Veff), orifice_eff.d
    ...
    *******************************
        ...
        printf(string)
        QMessageBox.information(MainWindow, "提示", "计算成功！",
                                QMessageBox.Yes, QMessageBox.Yes)

    except KeyError:
        QMessageBox.critical(MainWindow, "错误", "请输入需要计算的参数",
                             QMessageBox.Yes, QMessageBox.Yes)
    except ValueError as Arguement:
        QMessageBox.critical(MainWindow, "错误", str(Arguement).split(":")[
                             0], QMessageBox.Yes, QMessageBox.Yes)
    except RuntimeError:
        QMessageBox.critical(
            MainWindow, "错误", "程序将陷入死循环，请重新选择参数", QMessageBox.Yes, QMessageBox.Yes)


def show_picture():
    """[用于实现根据虚喷嘴模型的选择而事实更换展示窗中的图片]
    """
    try:
        if ui.NN_model.currentText() == 'Brich 1984模型':
            picture_name = r'.\brich84.png'
        elif ui.NN_model.currentText() == 'Brich 1987模型':
            picture_name = r'.\brich87.png'
        elif ui.NN_model.currentText() == 'Ewan 1986模型':
            picture_name = r'.\Ewan86.png'
        elif ui.NN_model.currentText() == 'Yuceil 2002模型':
            picture_name = r'.\Yuceil02.png'
        elif ui.NN_model.currentText() == 'Molkov 模型':
            picture_name = r'.\Molkov.png'
        else:
            printf('正在开发')
        Qpixmap = QtGui.QPixmap(picture_name)
        ui.NN_picture.setPixmap(Qpixmap)
        ui.NN_picture.setScaledContents(True)
    except:
        pass


def save_txt():
    """[用于实现将计算结果保存为txt文件，路径为脚本运行根目录，文件名格式为气体_气体温度_出口直径_压力.txt]

    Raises:
        ValueError: [当没有计算结果时将会抛出错误，提示无法保存文件]
    """
    try:
        file_name = f"""{ui.NN_gas_name.currentText()}_{ui.NN_gasT_in.text()}{ui.NN_gasT.currentText().split('（')[1].split('）')[0]}_{ui.NN_dia_in.text()}{ui.NN_dia.currentText().split('（')[1].split('）')[0]}_{ui.NN_P_in.text()}{ui.NN_P.currentText().split('（')[1].split('）')[0]}.txt"""
        file_path = os.getcwd()
        if len(string) < 1:
            raise ValueError
        ...
        QMessageBox.information(
            MainWindow, "提示", "文件保存成功！", QMessageBox.Yes, QMessageBox.Yes)
    except:
        QMessageBox.critical(MainWindow, "错误", '无计算结果',
                             QMessageBox.Yes, QMessageBox.Yes)
...
#############################################信号连接#################################################
    ...
    ui.parameters_P.toggled.connect(
        lambda: radio_button(ui.parameters_P, ui.PA_P_in))
    ui.parameters_T.toggled.connect(
        lambda: radio_button(ui.parameters_T, ui.PA_T_in))
    ui.parameters_rho.toggled.connect(
        lambda: radio_button(ui.parameters_rho, ui.PA_rho_in))

    """实例化单位换算类"""
    conv = UnitConv()

    """直径单位换算及数值更新"""
    ui.NN_dia.currentIndexChanged.connect(
        lambda: conv.length_refresh(ui.NN_dia, ui.NN_dia_in, g_length_output))
    ui.NN_dia_in.editingFinished.connect(
        lambda: conv.length_Unit_conversion(ui.NN_dia, ui.NN_dia_in, g_length_output))

    """压力单位换算及数值更新"""
    ui.NN_P.currentIndexChanged.connect(
        lambda: conv.pressure_refresh(ui.NN_P, ui.NN_P_in, NN_pressure_output))
    ui.NN_P_in.editingFinished.connect(
        lambda: conv.pressure_Unit_conversion(ui.NN_P, ui.NN_P_in, NN_pressure_output))
    ui.PA_P.currentIndexChanged.connect(
        lambda: conv.pressure_refresh(ui.PA_P, ui.PA_P_in, PA_pressure_output))
    ui.PA_P_in.editingFinished.connect(
        lambda: conv.pressure_Unit_conversion(ui.PA_P, ui.PA_P_in, PA_pressure_output))

    """温度单位换算及数值更新"""
    ui.PA_T.currentIndexChanged.connect(lambda: conv.temperature_refresh(
        ui.PA_T, ui.PA_T_in, PA_temperature_output))
    ui.PA_T_in.editingFinished.connect(lambda: conv.temperature_unit_conversion(
        ui.PA_T, ui.PA_T_in, PA_temperature_output))
    ui.NN_ambT.currentIndexChanged.connect(lambda: conv.temperature_refresh(
        ui.NN_ambT, ui.NN_ambT_in, Tamb_temperature_output))
    ui.NN_ambT_in.editingFinished.connect(lambda: conv.temperature_unit_conversion(
        ui.NN_ambT, ui.NN_ambT_in, Tamb_temperature_output))
    ui.NN_gasT.currentIndexChanged.connect(lambda: conv.temperature_refresh(
        ui.NN_gasT, ui.NN_gasT_in, Tgas_temperature_output))
    ui.NN_gasT_in.editingFinished.connect(lambda: conv.temperature_unit_conversion(
        ui.NN_gasT, ui.NN_gasT_in, Tgas_temperature_output))

    """密度单位换算及数值更新"""
    ui.PA_rho.currentIndexChanged.connect(
        lambda: conv.rho_refresh(ui.PA_rho, ui.PA_rho_in, g_rho_output))
    ui.PA_rho_in.editingFinished.connect(
        lambda: conv.rho_unit_conversion(ui.PA_rho, ui.PA_rho_in, g_rho_output))
    ...
    """更新图片窗口的显示"""
    ui.NN_model.currentIndexChanged.connect(lambda: show_picture())
